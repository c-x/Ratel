...
4. You've got log files, you've got parsers to describe them, now load it into
Ratel Edit the ~/sources/sources.xml file to describe all log files you have.
...
5. Ratel can now read and parse your log files, now correlate !  Correlation
rules are defined in one or more xml files stored into the ~/rules directory. A
rule, has few mandatory fields :
...
Writing a condition : Ok, so, parsers with the "correlate" action will transform
...
Functions : A function is used into correlation rule. Each function must return
